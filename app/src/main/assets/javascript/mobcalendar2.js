...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        that.show = function (prevAnim) {
            ...
            // s.wheels[0]['日']=['2015年03月03日','2015年03月03日','2015年03月03日','2015年03月03日','2015年03月03日'];
...
(function ($) {
    ...
    var ms = $.mobiscroll,
        ...
        preset = function (inst) {
            ...
            if (p.match(/yeartime/i)) {
                // Determine the order of hours, minutes, seconds wheels
                ord = [];
                $.each(['wY','h', 'i', 's', 'a'], function (i, v) {
                    i = wyord.search(new RegExp(v, 'i'));
                    if (i > -1) {
                        ord.push({ o: i, v: v });
                    }
                });
                ...
                for (k = offset; k < offset + 4; k++) {
                    if(k == o.wY)
                    {
                        offset++;
                        w[s.wYText]={};
                        for(var daysi = 0;daysi< s.days;daysi++)
                        {
                            var _now = new Date(),
                                _nowtime = _now.getTime(),
                                _daytime = _nowtime + daysi*(24*60*60*1000),
                                _onow = new Date(_daytime),
                                _weekarr = ['日', '一', '二', '三', '四', '五', '六'],
                                _oweek = _now.getDay();
                            var _str = _onow.getFullYear()+'-'+((_onow.getMonth()+1)<=9?'0'+(_onow.getMonth()+1):(_onow.getMonth()+1))+'-'+(_onow.getDate()<=9?'0'+_onow.getDate():_onow.getDate());

                            w[s.wYText][_str] =  _onow.getMonth()+1+'月'+_onow.getDate()+'日 周'+_weekarr[(new Date(_daytime)).getDay()];
                            //      w['日'].push(_onow.getMonth+1+'月'+_onow.getDate()+'日 周'+_weekarr[(new Date(_daytime)).getDay()]);
                        }
                    }
                    if (k == o.h) {
                        offset++;
            ...
            return {
                 ...
                 */
                formatResult: function (d) {
                    var res = d[0]+' '+(d[1]<9?'0'+d[1]:d[1])+':'+(d[2]<9?'0'+d[2]:d[2])+':00';
    ...
    $.each(['date', 'time', 'datetime','yeartime'], function (i, v) {
        ms.presets[v] = preset;
        ms.presetShort(v);
    });
